...
It answers one narrow release question: *did this local build produce the runtime bytecode that is actually live at this contract address right now?*

## What a judge can verify in 30 seconds

1. Choose a Foundry artifact JSON file and paste a deployed Monad Testnet contract address.
2. ReleaseSeal reads the artifact's runtime bytecode locally and fetches the target's live bytecode from Monad.
3. It displays a deterministic exact-match or mismatch result, with both hashes and byte counts.
